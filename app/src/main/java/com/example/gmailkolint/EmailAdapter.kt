...
class Email(var sender: String, val time: String, val content: String, val rating: Boolean)
...
class EmailsAdaptor(private val emails: List<Email>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (convertView == null) {
            emailView =
                LayoutInflater.from(parent?.context).inflate(R.layout.custome_email_layout, parent, false)
        } else {
...
//        emailView.findViewById<RatingBar>(R.id.ratingBar).onRatingBarChangeListener.
